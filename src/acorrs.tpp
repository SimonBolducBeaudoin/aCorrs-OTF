...
// Methods //
template <class T> inline void ACorrUpTo<T>::accumulate(T *buffer, uint64_t size) {
...
template <class T> inline void ACorrUpTo<T>::accumulate_chunk(T *buffer, uint64_t size) {
    accumulate_m_rk(buffer, size); // Accumulating
    chunk_processed++;
}

template <class T> inline void ACorrUpTo<T>::accumulate_chunk_edge(T *buffer, uint64_t size) {
    accumulate_m_rk_edge(buffer, size); // Accumulating
                                        // chunk_processed++;
}
template <class T> inline void ACorrUpTo<T>::accumulate_m_rk(T *buffer, uint64_t size) {
#pragma omp parallel
    {
...
template <class T> inline void ACorrUpTo<T>::accumulate_m_rk_edge(T *buffer, uint64_t size) {
...
template <class T> inline void ACorrUpTo<T>::accumulate_gk(T *buffer, uint64_t size) {
    for (int i = 0; i < k; i++) {
        for (int j = 0; j < i; j++) {
...
template <class T> inline void ACorrUpTo<T>::accumulate_bk(T *buffer, uint64_t size) {
    for (int i = 0; i < k; i++) {
...
template <class T> inline double ACorrUpTo<T>::get_mean() { return (double)get_mean_mpfr(); }
...
template <class T> inline double ACorrUpTo<T>::get_var() { return (double)get_var_mpfr(); }
