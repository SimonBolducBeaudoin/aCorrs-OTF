...
// Methods //
template <class T> inline void ACorrUpToPhi<T>::accumulate(T *buffer, uint64_t size) {
...
template <class T> inline void ACorrUpToPhi<T>::accumulate_chunk(T *buffer, uint64_t size) {
    accumulate_mf_rfk(buffer, size); // Accumulating
    chunk_processed++;
}

template <class T> inline void ACorrUpToPhi<T>::accumulate_chunk_edge(T *buffer, uint64_t size) {
    accumulate_mf_rfk_edge(buffer, size); // Accumulating
                                          // chunk_processed++;
}

template <class T> uint64_t ACorrUpToPhi<T>::get_nfk(uint64_t N, int lambda, int f, int k) {
...
template <class T> inline void ACorrUpToPhi<T>::accumulate_mf_rfk(T *buffer, uint64_t size) {
#pragma omp parallel
    {
...
template <class T> inline void ACorrUpToPhi<T>::accumulate_mf_rfk_edge(T *buffer, uint64_t size) {
    for (int i = 0; i < k; i++) {
        for (int f = 0; f < lambda; f++) {
            ...
            for (uint64_t j = nfk[lambda * k - 1]; j < nfk[f * k + i]; j++) {
                ...
                rfk[f * k + i] +=
                    (accumul_t)buffer[f + j * lambda] * (accumul_t)buffer[f + j * lambda + i];
            }
        }
    }
}
...
template <class T> inline void ACorrUpToPhi<T>::accumulate_gfk(T *buffer, uint64_t size) {
    for (int f = 0; f < lambda; f++) {
...
template <class T> inline void ACorrUpToPhi<T>::accumulate_bfk(T *buffer, uint64_t size) {
    for (int f = 0; f < lambda; f++) {
        uint64_t alphaf = size / lambda + (f < (int)(size % lambda));
        for (int i = 0; i < k; i++) {
...
// It should be computed ONCE per block on the very first chunk of data
template <class T> inline void ACorrUpToPhi<T>::accumulate_gk(T *buffer, uint64_t size) {
    for (int i = 0; i < k; i++) {
        for (int j = 0; j < i; j++) {
...
// It should be computed ONCE per block on the very last chunk of data
template <class T> inline void ACorrUpToPhi<T>::accumulate_bk(T *buffer, uint64_t size) {
    for (int i = 0; i < k; i++) {
...
template <class T> inline mpreal *ACorrUpToPhi<T>::get_aCorrs_mpfr() {
    ...
    for (int i = 0; i < k; i++) {
        for (int f = 0; f < lambda; f++) {
            ...
            aCorrs_mpfr[f * k + i] =
                (rfk_mpfr[f * k + i] - (mf_mpfr[f] - bfk_mpfr[f * k + i]) *
...
template <class T> inline void ACorrUpToPhi<T>::get_aCorrs(double *res, int size) {
    compute_aCorrs();
