...
template <typename T> void declare_class(py::module &m, std::string typestr) {
    ...
    py::class_<Class>(m, pyclass_name.c_str(), py::buffer_protocol(), py::dynamic_attr())
        ...
        .def_property_readonly("m", [](Class &self) { return self.m_mpfr.toString(); })
        .def_property_readonly("n", [](Class &self) { return self.n_mpfr.toString(); })
...
template <typename T> void declare_fftclass(py::module &m, std::string typestr) {
...
template <typename T> void declare_phiclass(py::module &m, std::string typestr) {
    using Class = ACorrUpToPhi<T>;
    std::string pyclass_name = std::string("ACorrUpToPhi_") + typestr;
    py::class_<Class>(m, pyclass_name.c_str(), py::buffer_protocol(), py::dynamic_attr())
        .def(py::init<int, int>())
        .def("get_nfk",
             [](Class &self, uint64_t a, int b, int c, int d) { return self.get_nfk(a, b, c, d); })
        .def(
            "compute_current_nfk",
            [](Class &self, py::array_t<T, py::array::c_style> &array) {
                auto buff = array.request();
                ...
                auto res = py::array_t<uint64_t>(
                    ...
                    py::capsule(self.nfk, [](void *f) { ; })); // numpy array references this parent
        ...
        .def_property_readonly(
            ...
            [](Class &self) {
                ...
                return py::array_t<double>(
                    ...
                    py::capsule(self.ak, [](void *f) { ; })); // numpy array references this parent
            })
        .def_property_readonly("rfk",
                               [](Class &self) {
                                   vector<std::string> values;
                                   ...
                                   auto res =
                                       py::array(py::cast(values)); // auto saving my life here
                                   res.resize({self.lambda, self.k});
                                   return res;
                               })
        .def_property_readonly("nfk",
                               [](Class &self) {
                                   vector<std::string> values;
                                   for (int i = 0; i < self.lambda * self.k; i++) {
...
#undef declare_class_for
#undef declare_fftclass_for
#undef declare_phiclass_for
