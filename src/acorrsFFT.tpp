...
inline void halfcomplex_norm2(double *buff, int fftwlen) {
  ...
  for (j++; j < fftwlen / 2; j++) {
    // (a+ib)*(a-ib) = a²+b²
    buff[j] =
        buff[j] * buff[j] + buff[fftwlen - j] * buff[fftwlen - j]; // norm²
  }
...
template <class T>
inline ACorrUpToFFT<T>::ACorrUpToFFT(int k, int len)
    : ACorrUpTo<T>(k), len(len) {
  // FFT length
...
template <class T> inline int ACorrUpToFFT<T>::compute_accumul_max() {
  ...
  int ret = (int)min((((uint64_t)1) << 53) /
                         ((uint64_t)fftwlen * buff_max * buff_max),
                     (uint64_t)4096) *
            3 / 4;
  return ret;
}
...
#pragma omp for reduction(+ : m), reduction(+ : rk[:k])
    for (uint64_t i = 0; i < fftnum; i++) {
      ...
      for (j = 0; j < k; j++) {
        ...
        for (int l = j; l < k - 1; l++) {
          rk[l + 1] +=
              (accumul_t)buff[len - j - 1] * (accumul_t)buff[len - j + l];
        }
      }
...
#pragma omp critical
        for (int i = 0; i < k; i++) {
          ...
          rk_mpfr[i] += mpfr::rint_round(mpfr::rint_round((mpreal)obuff[i]) /
                                         (mpreal)fftwlen);
        }
